...
## MovieLens 1M
电影评分数据集（经典的轻量级基准数据集，数据结构清晰，适合推荐算法的基础验证与实验）

### 数据集说明
1. 该数据集由美国明尼苏达大学的GroupLens研究组发布于2003年2月，是推荐系统领域的经典稳定基准数据集。
2. 包含6040名2000年加入MovieLens平台的用户对约3900部电影的1000209条匿名评分数据，同时附带用户人口统计信息、电影基础信息，单用户至少包含20条评分记录，数据集压缩包大小为6MB。

### 数据格式
1. 评分数据文件ratings.dat：`UserID::MovieID::Rating::Timestamp`，以双冒号分隔
   - UserID范围1-6040，MovieID范围1-3952
   - 评分为五星制（仅整数评分）
   - 时间戳为从纪元开始的秒数
2. 用户信息文件users.dat：`UserID::Gender::Age::Occupation::Zip-code`，以双冒号分隔
   - 性别以M（男）、F（女）表示
   - 年龄划分为7个区间：1(18岁以下)、18(18-24)、25(25-34)、35(35-44)、45(45-49)、50(50-55)、56(56岁以上)
   - 职业包含21类可选，如学术/教育工作者、程序员、艺术家等，以0-20的数字对应
3. 电影信息文件movies.dat：`MovieID::Title::Genres`，以双冒号分隔
   - 电影标题与IMDB一致，包含上映年份
   - 电影类型以竖线分隔，可选类型含动作、冒险、喜剧、科幻、惊悚等18类
   - 部分MovieID因重复/测试录入无对应电影，数据为人工录入可能存在少量误差

[数据集说明](https://files.grouplens.org/datasets/movielens/ml-1m-README.txt)  
