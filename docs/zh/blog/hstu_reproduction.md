...
- **时间感知数据预处理**：`examples/generative/data/ml-1m/preprocess_ml_hstu.py`
  - 支持自动下载 MovieLens-1M、生成 HSTU 序列数据和时间差特征
...
MovieLens-1M 的 HSTU 预处理脚本位于 `examples/generative/data/ml-1m/preprocess_ml_hstu.py`。默认行为是检查脚本所在目录是否已有 `ratings.dat`、`movies.dat` 和 `users.dat`；如果缺失，会自动下载官方 `ml-1m.zip` 并解压到同一目录。
...
# 只使用本地已有的 ratings.dat、movies.dat、users.dat
...
默认输出目录为 `examples/generative/data/ml-1m/processed/`，包含：

- `vocab.pkl`：MovieLens 原始 movie id 到连续 token id 的映射，`0` 保留为 padding；
- `train_data.pkl`、`val_data.pkl`、`test_data.pkl`：训练、验证、测试数据；
- 每个 split 文件都是字典，字段为 `seq_tokens`、`seq_positions`、`seq_time_diffs`、`targets`。

MovieLens 预处理按用户划分数据集，默认用户比例为 70% train、10% val、20% test，避免同一用户的样本同时出现在训练和测试中。
...
- 按时间戳排序用户交互，并将原始 movie id 映射为连续 token id；
...
- `SeqDataset` 负责将 `train_data.pkl`、`val_data.pkl`、`test_data.pkl` 中的 NumPy 数组转换为 PyTorch 张量；
...
- 重构 MovieLens 预处理脚本：
  - 支持自动下载官方 MovieLens-1M 数据；
  - 支持 `--no_download` 复用本地文件和 `--overwrite` 刷新下载/解压结果；
  - 使用滑动窗口策略大幅增加训练样本；
