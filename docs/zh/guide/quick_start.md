...
如果使用 NVIDIA CUDA、AMD ROCm 或华为昇腾 NPU，请先参考 [安装指南](/zh/guide/install) 中的 PyTorch 版本适配链接。

```bash
pip install torch         # 先安装适合当前设备的 PyTorch；CUDA/ROCm/NPU 环境请参考安装指南
...
trainer = CTRTrainer(
    ...
    n_epoch=2,
    device="cpu",  # NVIDIA/ROCm GPU 改成 "cuda:0"；NPU 可按环境改成 "npu:0"
...
- NVIDIA/ROCm GPU 训练：将 `device="cpu"` 改成 `device="cuda:0"`；NPU 环境可按实际配置改成 `device="npu:0"`
