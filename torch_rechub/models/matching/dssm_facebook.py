...
class FaceBookDSSM(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sim_func == "cosine":
            pos_score = torch.cosine_similarity(user_embedding, pos_item_embedding, dim=1)
            neg_score = torch.cosine_similarity(user_embedding, neg_item_embedding, dim=1)
    ...
    def item_tower(self, x):
        ...
        return self.item_mlp(input_item_pos), self.item_mlp(input_item_neg)